...
import { startAgent, stopAgent } from "./scheduler";
...
async function getOrCreateScraper(agent: Agent): Promise<{ scraper: Scraper; error?: string }> {
  ...
  if (cached && (Date.now() - cached.lastLogin) < LOGIN_CACHE_DURATION) {
    try {
      ...
      console.log('[Scraper] Cache check failed for agent, will re-login');
    }
  }

  // Check if login is already in progress for this agent (prevent duplicate concurrent logins)
  if (existingLoginPromise) {
    console.log('[Scraper] Login already in progress for agent, waiting...');
...
async function performScraperLogin(
  ...
  try {
    ...
    if (!hasLoginCredentials) {
      return {
        scraper: null as any,
        error: 'SCRAPER_CREDENTIALS_EXPIRED: Session cookies expired. For Free Tier stability, please re-import cookies from your browser using the "Import Cookies" feature.',
      };
    }
    ...
    // Clean up error message for common issues to prevent terminal spam
    if (lastError.includes('403') || lastError.includes('Forbidden')) {
      console.warn('[Scraper] ⚠️  Access Denied (403): Session valid but action blocked. Twitter may be limiting this IP or account temporarily.');
    } else if (lastError.includes('404')) {
      console.warn('[Scraper] ⚠️  Not Found (404): Could not find timeline or tweets. This is expected if using partial cookies.');
    } else {
      console.error('[Scraper] Login failed for ' + agent.name + ': ' + lastError);
    }
...
export async function fetchRepliesViaScraper(
  ...
  try {
    ...
    try {
      ...
      console.log('[Scraper] Successfully found ' + replies.length + ' direct replies to tweet ' + tweetId);
      return { success: true, replies };
...
export interface FetchMonitoredTweetsResult extends ScraperResult {
  tweets?: { username: string; tweet: ScrapedTweet }[];
}

/**
 * Fetch latest tweets from a list of monitored accounts
 */
export async function fetchMonitoredTweets(agent: Agent, targets: string[]): Promise<FetchMonitoredTweetsResult> {
  // Get scraper instance (using cookies or login)
  const { scraper, error } = await getOrCreateScraper(agent);
  if (!scraper || error) {
    return { success: false, error };
  }

  const results: { username: string; tweet: ScrapedTweet }[] = [];
  const errors: string[] = [];

  console.log('[NewsMonitor] Fetching updates for ' + agent.name + ' from targets: ' + targets.join(', '));

  // Shuffle targets to avoid hitting same order every time, take max 5 to enforce safety
  const safeTargets = targets.sort(() => 0.5 - Math.random()).slice(0, 5);

  for (const username of safeTargets) {
    try {
      // Add random delay between checks (5-10s) to be human-like
      await new Promise(r => setTimeout(r, 5000 + Math.random() * 5000));

      let tweetsIterator: AsyncGenerator<any> | null = null;
      try {
        tweetsIterator = scraper.getTweets(username, 3);
      } catch (err: any) {
        console.warn('[NewsMonitor] getTweets methods failed for @' + username + ', trying search fallback...');
        throw err;
      }

      // Convert iterator to array safely
      const fetchedTweets: any[] = [];
      try {
        for await (const tweet of tweetsIterator) {
          fetchedTweets.push(tweet);
          if (fetchedTweets.length >= 3) break;
        }
      } catch (iteratorError: any) {
        // Only trigger fallback if we got ZERO tweets and it errored
        if (fetchedTweets.length === 0) {
          throw iteratorError;
        }
        // If we got some tweets but then errored, just use what we have
        console.warn('[NewsMonitor] Partial fetch for @' + username + ': ' + iteratorError.message);
      }

      let count = 0;
      for (const tweet of fetchedTweets) {
        // Filter out replies and retweets - we want original news
      ...
      console.warn('[NewsMonitor] Cookie scraper failed for @' + username + ' (Error: ' + err.message + ')');

      // Apify Fallback
      if (process.env.APIFY_API_TOKEN) {
        console.log('[NewsMonitor] Attempting Apify fallback for @' + username + '...');
        try {
          // Use Apify to get recent tweets from this user
          const apifyTweets = await ApifyService.getUserTweets(username, 3);

          for (const tweet of apifyTweets) {
            ...
            if (tweet.text && !tweet.text.startsWith('RT @')) {
              results.push({
                ...
                tweet: {
                  ...
                  isRetweet: false, // Filtered above
                  likes: tweet.favorite_count,
                  retweets: tweet.retweet_count,
                  replies: tweet.reply_count,
                  permanentUrl: 'https://twitter.com/' + username + '/status/' + tweet.id,
                }
              });
            }
          }
          ...
          if (apifyTweets.length > 0) {
            console.log('[NewsMonitor] Apify fallback successful for @' + username + ' (' + apifyTweets.length + ' tweets)');
            continue; // Success, move to next target
          }
        } catch (apifyErr) {
          console.error('[NewsMonitor] Apify fallback also failed for @' + username + ':', apifyErr);
        }
      }

      // FALLBACK: Try search if profile fetch fails
      try {
        console.log('[NewsMonitor] Attempting fallback search for @' + username + '...');
        // Note: agent-twitter-client searchTweets doesn't always return iteration same way, use loop
        const searchIter = scraper.searchTweets('from:' + username, 5, SearchMode.Latest);

        let searchCount = 0;
        for await (const tweet of searchIter) {
          if (searchCount >= 2) break;
          // Ensure it's actually from the user (search can sometimes be fuzzy)
          if (tweet.username.toLowerCase() === username.toLowerCase() && !tweet.isReply && !tweet.isRetweet) {
            results.push({
              username: username,
              tweet: {
                id: tweet.id,
        ...
        if (searchCount > 0) {
          console.log('[NewsMonitor] Fallback search successful for @' + username + ' (' + searchCount + ' tweets)');
        }
      } catch (fallbackErr: any) {
        console.warn('[NewsMonitor] Fallback search also failed for @' + username + ': ' + fallbackErr.message);
        errors.push(username + ': ' + err.message + ' (Fallback: ' + fallbackErr.message + ')');
      }
    }
  }
  ...
  return {
    ...
    tweets: results.sort((a, b) => (b.tweet.timeParsed?.getTime() || 0) - (a.tweet.timeParsed?.getTime() || 0)), // Sort by newest
    // error: errors.length > 0 ? 'Some targets failed: ' + errors.join(', ') : undefined
  };
}
