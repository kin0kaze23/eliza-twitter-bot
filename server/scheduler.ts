...
interface SchedulerState {
  ...
  // News Monitor State
  latestNewsContext: Map<string, ScrapedTweet[]>;
  lastNewsCheck: Map<string, Date>;
...
const state: SchedulerState = {
  ...
  postingLock: new Map(),
  latestNewsContext: new Map(), // NEW
  lastNewsCheck: new Map(), // NEW
};
...
// Tweaked for Free Tier Stability (approx 50 posts/24h)
const MIN_POSTING_INTERVAL_MS = 60 * 60 * 1000; // 60 minutes minimum (was 30)
const RECOMMENDED_POSTING_INTERVAL_MS = 90 * 60 * 1000; // 1.5 hours recommended
const FREE_TIER_DAILY_LIMIT = 40; // Safety buffer below strict 50 limit

// Track total daily usage (posts + replies)
function getDailyUsage(agentId: string): number {
  const today = new Date().toISOString().split("T")[0];
  const postKey = `${agentId}_${today}`;
  const replies = state.repliesThisHour.get(agentId)?.count || 0; // Approximate daily replies with hourly (imperfect but safe)
  // Ideally we should track daily replies separately, but for now we'll rely on the post counter + aggressive hourly limits
  // Better: Let's simply track a "dailyTotal" in memory for the runtime

  const dailyTotal = state.postsToday.get(postKey) || 0;
  return dailyTotal;
}

function checkDailyLimit(agentId: string, limit: number = FREE_TIER_DAILY_LIMIT): boolean {
  const usage = getDailyUsage(agentId);
  if (usage >= limit) {
    console.log(`[Scheduler] Daily limit reached for ${agentId} (${usage}/${limit}). Pausing activity.`);
    return false;
  }
  return true;
}
...
function canPostNow(agent: Agent): boolean {
  if (!agent.postingEnabled) {
    console.log(`[DEBUG] canPostNow: posting disabled`);
    return false;
  }
  if (isInQuietHours(agent)) {
    console.log(`[DEBUG] canPostNow: in quiet hours`);
    return false;
  }
  ...
  if (backoffUntil && Date.now() < backoffUntil.getTime()) {
    console.log(`[DEBUG] canPostNow: in rate limit backoff`);
    return false;
  }

  // Check if already posting (prevent duplicates)
  if (state.postingLock.get(agent.id)) {
    console.log(`[DEBUG] canPostNow: posting lock held`);
    return false;
  }
  ...
  if (postsToday >= maxPostsPerDay) {
    console.log(`[DEBUG] canPostNow: daily limit reached (${postsToday}/${maxPostsPerDay})`);
    return false;
  }
  ...
  if (lastActivity) {
    ...
    if (timeSinceLastActivity < intervalMs) {
      console.log(`[DEBUG] canPostNow: interval not elapsed (${Math.round(timeSinceLastActivity / 1000)}s < ${Math.round(intervalMs / 1000)}s)`);
      return false;
    }
  }
...
/**
 * Select the next topic based on configured topics and recent usage
 */
async function selectNextTopic(agent: Agent): Promise<string | undefined> {
  // 1. Check if feature is enabled
  const topicAvoidanceEnabled = (agent as any).topicAvoidanceEnabled !== false; // Default true
  if (!topicAvoidanceEnabled) return undefined;

  // 2. Get configured topics
  if (!agent.topics || !agent.topics.trim()) return undefined;

  // Split by comma and clean up
  const configuredTopics = agent.topics
    .split(',')
    .map(t => t.trim())
    .filter(t => t.length > 0);

  if (configuredTopics.length === 0) return undefined;

  // 3. Get recently used topics
  const windowSize = (agent as any).topicAvoidanceWindow || 5;
  const recentUsages = await storage.getRecentTopicUsages(agent.id, windowSize);
  const recentTopics = new Set(recentUsages.map(u => u.topic.toLowerCase()));

  // 4. Filter available topics (exclude recently used)
  const availableTopics = configuredTopics.filter(t => !recentTopics.has(t.toLowerCase()));

  // 5. Select topic
  if (availableTopics.length > 0) {
    // Pick random from available
    const selected = availableTopics[Math.floor(Math.random() * availableTopics.length)];
    console.log(`[Scheduler] Selected fresh topic for ${agent.name}: "${selected}" (Available: ${availableTopics.length}/${configuredTopics.length})`);
    return selected;
  } else {
    // All topics used recently - reset cycle (pick any random topic)
    const selected = configuredTopics[Math.floor(Math.random() * configuredTopics.length)];
    console.log(`[Scheduler] All topics used recently. Resetting cycle for ${agent.name}. Selected: "${selected}"`);
    return selected;
  }
}

async function generateTweetContent(agent: Agent): Promise<{ content: string; kbIds: string[]; contentType: ContentType; topic?: string } | null> {
  try {
    ...
    // TOPIC SELECTION (Anti-Repetition)
    const focusTopic = await selectNextTopic(agent);

    // NEWS MONITOR (News Context Logic)
    let newsContext: ScrapedTweet[] = [];
    if (agent.monitoringEnabled) {
      const now = new Date();
      const lastCheck = state.lastNewsCheck.get(agent.id) || new Date(0);
      const intervalHours = agent.monitoringIntervalHours || 4;

      // Check if we need to refresh news (every X hours)
      if (now.getTime() - lastCheck.getTime() > intervalHours * 60 * 60 * 1000) {
        console.log(`[NewsMonitor] Auto-refreshing news for ${agent.name}...`);
        await runNewsMonitor(agent);
      }

      // Always get the current context from state (whether just refreshed or cached)
      newsContext = state.latestNewsContext.get(agent.id) || [];
    }
    ...
    // Priority Mode Removed - Focusing on Topic Rotation
    ...
    const assembledPrompt = await assemblePrompt(agent, knowledgeEntries, {
      ...
      focusTopic, // NEW: Specific topic to focus on
      newsContext, // NEW: Injected news context from News Monitor
    });
...
OUTPUT: Write ONLY the tweet content, nothing else.`;
    ...
    if (postModelProvider === "openai") {
      console.log(`[DEBUG] Initializing OpenAI for model: ${postModelName}`);
    ...
    console.error(`[DEBUG] Failed to generate tweet for agent ${agent.id}. Error: ${error.message} Stack: ${error.stack}`);
    return null;
  }
}
...
function scheduleAgent(agent: Agent): void {
  ...
  if (persistedLastPost) {
    ...
    console.log(`[DEBUG] Hydrated lastPostTime from Persistence: ${persistedLastPost}`);
  }
...
async function processMention(agent: Agent, mention: TwitterMention, mentionType: 'mention' | 'reply' = 'mention'): Promise<void> {
  ...
  try {
    ...
    if (result.success && result.tweetId) {
      ...
      // Increment reply count on success
      incrementReplyCount(agent.id);

      console.log(`[MentionBot] Reply sent! Total for hour: ${state.repliesThisHour.get(agent.id)?.count || 1}`);
    } else {
...
async function pollMentions(agent: Agent): Promise<void> {
  ...
  if (!agent.replyEnabled) {
    console.log(`[MentionBot] Reply feature disabled for ${agent.name}, skipping polling`);
    return;
  }

  // CHECK 2: Hourly Rate Limit
  const maxReplies = agent.maxRepliesPerHour || 5;
  if (!canReplyThisHour(agent.id, maxReplies)) {
    const replyData = state.repliesThisHour.get(agent.id);
    console.log(`[MentionBot] Rate limit reached for ${agent.name} (${replyData?.count || maxReplies}/${maxReplies} replies this hour). Skipping polling.`);
    return;
  }
  ...
  const today = new Date().toISOString().split("T")[0];
  const postsKey = `${agent.id}_${today}`;
  const dailyCount = state.postsToday.get(postsKey) || 0;
  if (dailyCount >= FREE_TIER_DAILY_LIMIT) {
    console.log(`[MentionBot] Daily limit (${FREE_TIER_DAILY_LIMIT}) reached. Skipping mention polling.`);
    return;
  }
  ...
  try {
    ...
    // FREE TIER OPTIMIZATION: Always prefer Scraper for reading if available
    const preferScraper = hasScraper;

    if (hasApi && !preferScraper) {
      // Use Twitter API (OAuth) - only if we have no scraper option
      ...
      if (result.success) {
        ...
        // Fallback to scraper logic below if available
      }
    }

    // If no mentions found yet (API failed or skipped), try scraper
    if (mentions.length === 0 && hasScraper) {
      // Only log if we didn't try API first or API failed
      if (!hasApi || preferScraper) {
        // console.log(`[MentionBot] Using Scraper for reading mentions (Free Tier optimization)`);
      } else {
      ...
      if (scraperResult.success && scraperResult.mentions) {
        ...
        const isCookieError = scraperResult.error?.includes("cookies") || scraperResult.error?.includes("Login");
        if (!isCookieError) {
    ...
    if (mentions.length > 0) {
      ...
      // Update the last mention ID for pagination
      if (newestId) {
...
export function getAgentStatus(agentId: string) {
  ...
  // Calculate next run time (approximate) - Safety check for maps
  const isRunning = state?.activeAgents?.has(agentId) ?? false;
  const lastPostTime = state?.lastPostTime?.get(agentId);
  const postsToday = state?.postsToday?.get(postKey) || 0;
  const rateLimitResetAt = state?.rateLimitBackoff?.get(agentId);
  const isRateLimited = (rateLimitResetAt?.getTime() || 0) > Date.now();
...
export function getLatestNewsContext(agentId: string) {
  return state.latestNewsContext.get(agentId) || [];
}

/**
 * Manually trigger the news scraper for an agent
 * Returns the number of new tweets found
 */
export async function runNewsMonitor(agent: Agent): Promise<{ success: boolean; count: number }> {
  if (!agent.monitoringEnabled) {
    return { success: false, count: 0 };
  }

  console.log(`[NewsMonitor] Starting manual news check for ${agent.name}...`);
  const targets = (agent.monitoringTargets || "").split(",").map(t => t.trim()).filter(t => t.length > 0);

  if (targets.length === 0) {
    console.log(`[NewsMonitor] No monitoring targets configured.`);
    return { success: false, count: 0 };
  }

  try {
    const result = await fetchMonitoredTweets(agent, targets);
    const now = new Date();

    if (result.success && result.tweets && result.tweets.length > 0) {
      console.log(`[NewsMonitor] Successfully fetched ${result.tweets.length} new tweets.`);
      const tweets = result.tweets.map((t: any) => t.tweet);
      // Ensure latestNewsContext is a Map before setting
      if (!state.latestNewsContext) {
        state.latestNewsContext = new Map();
      }
      state.latestNewsContext.set(agent.id, tweets);
      if (!state.lastNewsCheck) {
        state.lastNewsCheck = new Map();
      }
      state.lastNewsCheck.set(agent.id, now);
      return { success: true, count: tweets.length };
    } else {
      console.log(`[NewsMonitor] No new tweets found or fetch failed.`);
      if (!state.lastNewsCheck) {
        state.lastNewsCheck = new Map();
      }
      state.lastNewsCheck.set(agent.id, now);
    ...
    console.error(`[NewsMonitor] Error during manual check:`, error);
    return { success: false, count: 0 };
  }
}
