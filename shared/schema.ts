...
export const agents = pgTable("agents", {
  ...
  postExamples: jsonb("post_examples").$type<string[]>().default(sql`'[]'`), // New parity field
  lore: text("lore"), // New parity field
  chatStyle: text("chat_style"), // New parity field
  styleAll: text("style_all"), // New parity field
  ...
  // Knowledge Base Auto-Refresh
  kbAutoRefreshEnabled: boolean("kb_auto_refresh_enabled").default(false),
  kbAutoRefreshIntervalHours: integer("kb_auto_refresh_interval_hours").default(24),
  ...
  // Content modules (Simplified - Legacy toggles removed, use 'topics' field instead)
  // cryptoCommentary, marketAnalysis, etc removed in favor of general topics
  ...
  // KB & Event Priority Settings (Simplified - Removed complex priority logic)
  ...
  monitoringTargets: text("monitoring_targets").default("DegenerateNews,Cointelegraph,TheInsiderPaper,FirstSquawk"),
  monitoringIntervalHours: integer("monitoring_interval_hours").default(4),
  ...
  // Topic Diversity Tracking - NEW: prevent repetitive topics
  topicAvoidanceEnabled: boolean("topic_avoidance_enabled").default(true),
  topicAvoidanceWindow: integer("topic_avoidance_window").default(5), // number of posts to look back
...
export const insertAgentSchema = createInsertSchema(agents).omit({
  ...
  kbAutoRefreshEnabled: z.boolean().optional(),
  kbAutoRefreshIntervalHours: z.coerce.number().int().min(1).max(168).optional(),
});
...
// Topic Usage Tracking - prevents repetitive topics
export const topicUsages = pgTable("topic_usages", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  agentId: varchar("agent_id").notNull().references(() => agents.id, { onDelete: "cascade" }),
...
export const insertTopicUsageSchema = createInsertSchema(topicUsages).omit({
  id: true,
  createdAt: true,
});

export type InsertTopicUsage = z.infer<typeof insertTopicUsageSchema>;
export type TopicUsage = typeof topicUsages.$inferSelect;
